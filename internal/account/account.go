...
type Account struct {
	...
	ProjectID        string
	ExpiresAt        int64
	QuotaRemaining   *int64 // nil if unknown (NULL)
	QuotaFetchedAt   *int64 // nil if NULL
	QuotaJSON        string // empty if not yet fetched
	QuotaSummary     string // empty if not yet fetched
	ProtectedModels  []string
	LastUsedAt       *int64 // nil if never used
	LastError        string
	OperatorDisabled bool
	HealthDisabled   bool
	CreatedAt        int64
	MachineID        string // per-account machine ID for upstream fingerprint
}

// Disabled reports whether the account is schedulable.
