...
type FetchedQuota struct {
	...
	SummaryBlob      string
	ModelMetas       map[string]ModelMeta // keyed by lowercase model name
}

// ModelMeta holds the per-model metadata that AGY exposes via
// fetchAvailableModels. This is used by the proxy to set correct
// thinkingBudget and maxOutputTokens without hardcoding.
...
func FetchQuotaCtx(ctx context.Context, client *http.Client, accessToken, projectID string) (*FetchedQuota, error) {
	...
	modelMetas := parseModelMetas(modelsBody)
...
// parseModelMetas extracts per-model metadata (thinkingBudget, maxOutputTokens,
// supportsThinking, etc.) from the fetchAvailableModels response.
func parseModelMetas(body string) map[string]ModelMeta {
	var parsed struct {
		Models map[string]json.RawMessage `json:"models"`
	}
	if err := json.Unmarshal([]byte(body), &parsed); err != nil {
	...
	out := make(map[string]ModelMeta, len(parsed.Models))
	for name, info := range parsed.Models {
		var meta ModelMeta
		_ = json.Unmarshal(info, &meta)
		out[strings.ToLower(name)] = meta
	}
	return out
}
