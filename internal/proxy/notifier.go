...
type Notifier interface {
	...
	// NotifyProviderUnhealthy is called when a provider fails its
	// connectivity probe.
	NotifyProviderUnhealthy(name string, reason string)
	// NotifyProviderRecovered is called when a previously unhealthy
	// provider passes its connectivity probe again.
	NotifyProviderRecovered(name string)
}
...
type LogNotifier struct {
	...
	providerUnhealthy map[string]bool
}
...
func NewLogNotifier() *LogNotifier {
	return &LogNotifier{
		...
		providerUnhealthy: make(map[string]bool),
	}
}
...
func (n *LogNotifier) NotifyProviderUnhealthy(name string, reason string) {
	n.mu.Lock()
	wasUnhealthy := n.providerUnhealthy[name]
	n.providerUnhealthy[name] = true
	n.mu.Unlock()
	if wasUnhealthy {
		return
	}
	log.Printf("[NOTIFY] provider %s UNHEALTHY: %s", name, reason)
}

func (n *LogNotifier) NotifyProviderRecovered(name string) {
	n.mu.Lock()
	wasUnhealthy := n.providerUnhealthy[name]
	delete(n.providerUnhealthy, name)
	n.mu.Unlock()
	if !wasUnhealthy {
		return
	}
	log.Printf("[NOTIFY] provider %s RECOVERED", name)
}
