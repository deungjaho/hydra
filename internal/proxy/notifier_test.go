...
type mockNotifier struct {
	...
	provUnhealthy    []string
	provRecovered    []string
}
...
func (m *mockNotifier) NotifyProviderUnhealthy(name string, reason string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.provUnhealthy = append(m.provUnhealthy, name)
}

func (m *mockNotifier) NotifyProviderRecovered(name string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.provRecovered = append(m.provRecovered, name)
}
