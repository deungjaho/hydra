...
import (
	...
	"github.com/deungjaho/hydra/internal/keychainmigration"
)
...
func newMigrateKeychainCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "migrate-keychain",
		Short: "Migrate secrets from DB to OS keychain",
		Long: `Migrate all plaintext secrets (OAuth tokens, provider API keys,
client API keys) from the SQLite database to the OS keychain.

After migration, the DB columns contain a sentinel value and the real
secrets are stored securely in:
  - macOS: Keychain
  - Linux: Secret Service (D-Bus)
  - Windows: Credential Manager

If the keychain is unavailable (e.g., headless Linux without D-Bus),
secrets remain in the DB and the command reports what was skipped.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			svc, cleanup, err := newService(cmd)
			if err != nil {
				return err
			...
			result, err := keychainmigration.Migrate(svc.DB)
			if err != nil {
				return app.AsAppError(err)
			}

			r := getRenderer(cmd)
			return r.WriteSuccess(map[string]any{
				"accounts_migrated":  result.AccountsMigrated,
				"providers_migrated": result.ProvidersMigrated,
				"api_keys_migrated":  result.APIKeysMigrated,
				"skipped":            result.Skipped,
				"errors":             result.Errors,
			}, func(w io.Writer) error {
				fmt.Fprintf(w, "Keychain migration complete:\n")
				fmt.Fprintf(w, "  Accounts migrated:  %d\n", result.AccountsMigrated)
				fmt.Fprintf(w, "  Providers migrated: %d\n", result.ProvidersMigrated)
				fmt.Fprintf(w, "  API keys migrated:  %d\n", result.APIKeysMigrated)
				if result.Skipped > 0 {
					fmt.Fprintf(w, "  Skipped (keychain unavailable): %d\n", result.Skipped)
				}
				...
				if result.AccountsMigrated+result.ProvidersMigrated+result.APIKeysMigrated == 0 && result.Skipped == 0 {
					fmt.Fprintf(w, "  (nothing to migrate — all secrets already in keychain)\n")
				}
				return nil
			})
		},
