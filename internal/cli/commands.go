...
func NewRootCmd() *cobra.Command {
	...
	root.AddCommand(newMigrateKeychainCmd())
...
func newServeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			state := proxy.NewProxyState(d, cfg.Scheduling.MaxConcurrencyPerAccount)
...
func newAccountsCmd() *cobra.Command {
	...
	cmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useDevice {
				result, err = account.RunDeviceFlow(client)
			} else {
	...
	addCmd := cmd.Commands()[len(cmd.Commands())-1]
	addCmd.Flags().Bool("device", false, "use OAuth Device Flow (better for SSH/headless environments)")
	cmd.AddCommand(&cobra.Command{
		Use:   "list",
...
func newProvidersCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "providers",
		Short: "Manage API key providers (DeepSeek, Zhipu, etc.)",
	}
	cmd.AddCommand(&cobra.Command{
		Use:   "add",
		Short: "Add a new API key provider",
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			...
			if name == "" || baseURL == "" || apiKey == "" {
				return fmt.Errorf("--name, --base-url, and --api-key are required")
			}
			d, err := db.Open(config.DBPath())
			if err != nil {
				return err
			...
			defer d.Close()
			id, err := provider.AddProvider(d, name, baseURL, apiKey)
			if err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"id": id, "name": name, "base_url": baseURL}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Provider added: id=%d name=%s\n", id, name)
				return nil
			})
		},
	})
	addCmd := cmd.Commands()[0]
	addCmd.Flags().String("name", "", "provider name (e.g. deepseek)")
	addCmd.Flags().String("base-url", "", "provider base URL (e.g. https://api.deepseek.com)")
	...
	cmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return r.WriteSuccess(providers, func(w io.Writer) error {
				if len(providers) == 0 {
					fmt.Fprintln(w, "No providers configured. Run `hydra providers add` to add one.")
					return nil
				}
				fmt.Fprintf(w, "%-4s %-16s %-40s %-16s %-8s\n", "ID", "NAME", "BASE_URL", "API_KEY", "STATUS")
				for _, p := range providers {
					status := "active"
					if p.Disabled() {
						status = "disabled"
					}
					fmt.Fprintf(w, "%-4d %-16s %-40s %-16s %-8s\n",
						p.ID, p.Name, p.BaseURL, provider.MaskAPIKey(p.APIKey), status)
				}
				return nil
			})
		},
	...
	cmd.AddCommand(&cobra.Command{
		Use:   "remove",
		Short: "Remove a provider by id or name",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			...
			defer d.Close()
			id, err := resolveProviderID(d, args[0])
			if err != nil {
				return err
			}
			if err := provider.RemoveProvider(d, id); err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"id": id, "removed": true}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Provider removed: id=%d\n", id)
				return nil
			})
		},
	})
	...
	cmd.AddCommand(&cobra.Command{
		Use:   "test",
		Short: "Test connectivity to a provider by id or name",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			...
			p, err := provider.GetProvider(d, id)
			if err != nil {
				return err
			}
			// Simple connectivity test: GET /v1/models
			...
			if err != nil {
				return r.WriteError("UNAVAILABLE", fmt.Sprintf("provider %s unreachable: %v", p.Name, err), true, nil)
			}
			defer resp.Body.Close()
			if resp.StatusCode >= 200 && resp.StatusCode < 300 {
				return r.WriteSuccess(map[string]any{"id": id, "name": p.Name, "status": "ok"}, func(w io.Writer) error {
					fmt.Fprintf(w, "✓ Provider %s (id=%d) is reachable (HTTP %d)\n", p.Name, id, resp.StatusCode)
					return nil
				})
			}
			return r.WriteError("UNAVAILABLE", fmt.Sprintf("provider %s returned HTTP %d", p.Name, resp.StatusCode), true, nil)
		},
	})

	cmd.AddCommand(&cobra.Command{
		Use:   "enable",
		Short: "Enable a provider by id or name",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			...
			if err := provider.SetProviderDisabled(d, id, false); err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"id": id, "enabled": true}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Provider enabled: id=%d\n", id)
				return nil
			})
		},
	})
	...
	cmd.AddCommand(&cobra.Command{
		Use:   "disable",
		Short: "Disable a provider by id or name",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			...
			if err := provider.SetProviderDisabled(d, id, true); err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"id": id, "disabled": true}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Provider disabled: id=%d\n", id)
				return nil
			})
		},
	})

	// model subcommand for managing model → provider mappings
	modelCmd := &cobra.Command{
		Use:   "model",
		Short: "Manage model → provider mappings",
	}
	modelCmd.AddCommand(&cobra.Command{
		Use:   "add",
		Short: "Map a model ID to a provider",
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			...
			maxOut, _ := cmd.Flags().GetInt64("max-output")
			...
			defer d.Close()
			p, err := provider.GetProviderByName(d, providerName)
			if err != nil {
				return fmt.Errorf("provider %s not found: %w", providerName, err)
			}
			id, err := provider.AddModelMapping(d, modelID, p.ID, displayName, ctxWindow, maxOut, 0)
			if err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"id": id, "model_id": modelID, "provider": providerName}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Model mapping added: %s → %s\n", modelID, providerName)
				return nil
			})
		},
	})
	modelAddCmd := modelCmd.Commands()[0]
	modelAddCmd.Flags().String("id", "", "model ID (e.g. deepseek-chat)")
	modelAddCmd.Flags().String("provider", "", "provider name")
	modelAddCmd.Flags().String("display-name", "", "display name for the model")
	modelAddCmd.Flags().Int("context-window", 0, "context window size")
	modelAddCmd.Flags().Int64("max-output", 0, "max output tokens")

	modelCmd.AddCommand(&cobra.Command{
		Use:   "list",
		Short: "List all model → provider mappings",
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			if err != nil {
			...
			providers, _ := provider.ListProviders(d)
			provName := make(map[int64]string)
			for _, p := range providers {
				provName[p.ID] = p.Name
			}
			return r.WriteSuccess(mappings, func(w io.Writer) error {
				if len(mappings) == 0 {
					fmt.Fprintln(w, "No model mappings configured.")
					return nil
				}
				fmt.Fprintf(w, "%-4s %-24s %-16s %-20s %-10s %-10s\n", "ID", "MODEL", "PROVIDER", "DISPLAY", "CTX", "MAX_OUT")
				for _, m := range mappings {
					pn := provName[m.ProviderID]
					dn := m.DisplayName
					if dn == "" {
						dn = "-"
					}
					fmt.Fprintf(w, "%-4d %-24s %-16s %-20s %-10d %-10d\n",
						m.ID, m.ModelID, pn, dn, m.ContextWindow, m.MaxOutputTokens)
				}
				return nil
			})
		},
	...
	modelCmd.AddCommand(&cobra.Command{
		Use:   "remove",
		Short: "Remove a model mapping by model ID",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r := getRenderer(cmd)
			d, err := db.Open(config.DBPath())
			...
			defer d.Close()
			if err := provider.RemoveModelMapping(d, args[0]); err != nil {
				return err
			}
			return r.WriteSuccess(map[string]any{"model_id": args[0], "removed": true}, func(w io.Writer) error {
				fmt.Fprintf(w, "✓ Model mapping removed: %s\n", args[0])
				return nil
			})
		},
	})
...
// resolveProviderID resolves a string (numeric id or provider name) to a provider ID.
func resolveProviderID(d *db.Db, s string) (int64, error) {
	// Try numeric first.
	var id int64
	if _, err := fmt.Sscanf(s, "%d", &id); err == nil {
		// Verify it exists.
		if _, err := provider.GetProvider(d, id); err == nil {
			return id, nil
		}
	}
	// Try by name.
	p, err := provider.GetProviderByName(d, s)
	if err != nil {
		return 0, fmt.Errorf("provider %q not found", s)
	}
	return p.ID, nil
}
...
func newLogsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("%-6s %-20s %-4s %-24s %-8s %-8s %-15s %s\n",
				"ID", "TIME", "STAT", "MODEL", "PROMPT", "COMPL", "IP", "ERROR")
			...
			for _, l := range logs {
				...
				fmt.Printf("%-6d %-20s %-4d %-24s %-8d %-8d %-15s %s\n",
					l.ID, t, l.Status, model,
					derefOr(l.PromptTokens, 0),
					derefOr(l.CompletionTokens, 0),
					ip, errMsg)
			}
			return nil
		},
...
func newUsageCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "usage",
		Short: "Show usage stats aggregated from request logs",
		RunE: func(cmd *cobra.Command, args []string) error {
			d, err := db.Open(config.DBPath())
			if err != nil {
				return err
			...
			var totalPrompt, totalCompl, totalCached, totalThought, totalReqs int64
			...
			for _, r := range byModel {
				...
				fmt.Printf("%-28s %-6d %-10d %-10d %-10s %-10d\n",
					r.Label, r.Requests, r.PromptTokens, r.CompletionTokens,
					fmt.Sprintf("%d (%s)", r.CachedTokens, hit), r.ThoughtTokens)
			}
			fmt.Println()
			fmt.Println("By account:")
			fmt.Printf("%-28s %-6s %-10s %-10s %-10s %-10s\n",
				"EMAIL", "REQS", "PROMPT", "COMPL", "CACHED", "THINK")
			fmt.Println(strings.Repeat("-", 80))
...
func newKeyCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("%-4s %-14s %-20s %-8s %-8s %-10s %-12s\n",
				"ID", "LABEL", "KEY", "STATUS", "REQS", "TOKENS", "CREATED")
			...
			for _, k := range keys {
				...
				fmt.Printf("%-4d %-14s %-20s %-8s %-8d %-10d  %s  %s\n",
					k.ID, k.Label, prefix, status, reqs, tokens, created, schedInfo)
			}
			return nil
		},
	}
