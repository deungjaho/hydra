...
func (d *Db) migrate() error {
	...
	_, err := d.conn.Exec(`
        ...
        CREATE TABLE IF NOT EXISTS providers (
            ...
            health_disabled   INTEGER NOT NULL DEFAULT 0,
            last_error        TEXT,
	...
	// v10: per-account machine ID for upstream fingerprint isolation.
	if err := ensureColumn(d.conn, "accounts", "machine_id", "TEXT"); err != nil {
		return err
	}
	...
	// v11: multi-key failover — add priority to provider_models and
	// relax the UNIQUE(model_id) constraint to allow multiple providers
	// for the same model.
	if err := ensureColumn(d.conn, "provider_models", "priority", "INTEGER NOT NULL DEFAULT 0"); err != nil {
		return err
	}
	if userVersion < 11 {
		// Recreate provider_models without the UNIQUE(model_id) constraint.
		...
		if _, err := d.conn.Exec(`
			...
			INSERT INTO provider_models_new (id, model_id, provider_id, display_name, context_window, max_output_tokens, priority)
			SELECT id, model_id, provider_id, display_name, context_window, max_output_tokens, 0 FROM provider_models;
		...
		if _, err := d.conn.Exec("PRAGMA user_version = 11"); err != nil {
			return err
		}
	}
	if userVersion < 9 {
	...
	// v12: model enable/disable — add disabled column to provider_models.
	if err := ensureColumn(d.conn, "provider_models", "disabled", "INTEGER NOT NULL DEFAULT 0"); err != nil {
		return err
	}
	if userVersion < 12 {
