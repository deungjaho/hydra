...
type AppConfig struct {
	...
	APIProviders    []APIProviderConfig   `toml:"api_providers"`
	Models          []ModelEntry          `toml:"models"`
}

// APIProviderConfig declares an OpenAI-compatible provider that authenticates
// with a static API key (DeepSeek, Zhipu, Moonshot, xAI, etc.).
...
// ModelEntry registers a model in the unified catalog. Models not listed
// here fall through to the Antigravity path.
...
type SchedulingConfig struct {
	...
	// MaxConcurrencyPerAccount limits the number of in-flight upstream
	// requests per account. 0 = unlimited (no concurrency control).
	MaxConcurrencyPerAccount int `toml:"max_concurrency_per_account"`
	// QuotaWarnPercentage is the threshold below which an account is
	// deprioritized in scheduling. 0 = no early warning (only the
	// hard quota protection threshold applies).
	QuotaWarnPercentage int `toml:"quota_warn_percentage"`
}
...
func defaultScheduling() SchedulingConfig {
	return SchedulingConfig{
		Mode:                     SchedulingBalance,
		StickySessions:           true,
		MaxConcurrencyPerAccount: 0, // 0 = unlimited by default
		QuotaWarnPercentage:      20,
	}
}

// defaultMonitoredModels returns an empty list — quota protection monitors
// all models dynamically fetched from AGY. An empty list means "monitor all".
